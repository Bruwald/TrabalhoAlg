...
int codigo_item(ITEM *iteem){
	...
	if(iteem != NULL){													// caso seja um item valido retorna seu código
		return iteem->codigo;
	}
	...
	int numpalavras_item(ITEM *iteem){									// caso seja um item valido retorna o numero de palavras chaves
...
int relevancia_item(ITEM *iteem){
	...
	if(iteem != NULL){
		return iteem->relevancia;										// caso seja um item valido retorna sua relevancia
	}
...
int mudar_relevancia(ITEM *iteem, int relevancia){
	...
	if(iteem != NULL){
		iteem->relevancia = relevancia;									// recebe os dados e atualiza a relevancia
		return 1;
	}
...
int adicionar_palavra_chave(ITEM *iteem, char *palavranova){
	...
	if(iteem != NULL){
		...
		iteem->palavras = (char**) realloc (iteem->palavras, sizeof(char*) * (iteem->numpalavras + 1) );  // recebe os dados e adiciona uma nova
		iteem->palavras[iteem->numpalavras] = palavranova;												  // palavra
		iteem->numpalavras++;
